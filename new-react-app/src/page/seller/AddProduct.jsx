                    ...
                    <div className="flex flex-wrap items-center gap-3 mt-2">
                        {Array(4).fill('').map((_, index) => (
                            <label key={index} htmlFor={`image${index}`}>
                                ...
                                <img className="max-w-24 cursor-pointer" src={files[index] ? URL.createObjectURL(files[index]) : assets.images.upload_area} alt="uploadArea" width={100} height={100} />
                            </label>
                        ))}
                    </div>
                </div>
...
<option key={index} value={item.path}>{item.path}</option>
                ...
                <button className="px-8 py-2.5 bg-[#8DA100]  text-white font-medium rounded cursor-pointer">ADD</button>
            </form>
        </div>
    );
};
